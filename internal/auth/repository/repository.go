...
func (r *repoImpl) GetUserByEmail(email string) (*model.User, error) {
	...
	return &model.User{ID: user.ID, Email: user.Email, Name: func() string { v, _ := user.Name(); return v }()}, nil
}
...
func (r *repoImpl) CreateUser(email, name string) (*model.User, error) {
	...
	return &model.User{ID: usr.ID, Email: usr.Email, Name: func() string { v, _ := usr.Name(); return v }()}, nil
}
